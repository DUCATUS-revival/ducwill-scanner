...
public class ERC20PaymentMonitor {
    ...
    @Value("${io.lastwill.eventscan.contract.token-address.usdc}")
    private String tokenAddressUsdc;

    private final Map<String, CryptoCurrency> addressToCurrency = new HashMap<>();

    @PostConstruct
    protected void init() {
        addressToCurrency.put(tokenAddressUsdc.toLowerCase(), CryptoCurrency.USDC);
        ...
        for (final WrapperTransaction transaction : transactions) {
            ...
            transactionProvider.getTransactionReceiptAsync(networkType, transaction)
                    ...
                    .exceptionally(throwable -> {
                        log.error("Error on getting receipt for handling ERC20 payment.", throwable);
                        return null;
                    });
